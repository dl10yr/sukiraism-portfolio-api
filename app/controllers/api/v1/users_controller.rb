  ...
  module V1
    class UsersController < ApplicationController
      ...
      def answered_suki_posts
        ...
        answered_suki_posts = Post.where("id IN (#{answered_suki_posts_id})", user_id: @user.id).page(params[:page] ||= 1).per(10).order('created_at DESC')
        page_length = Post.where("id IN (#{answered_suki_posts_id})", user_id: @user.id).page(params[:page] ||= 1).per(10).total_pages
        json_data = {
      ...
      def answered_kirai_posts
        ...
        answered_kirai_posts = Post.where("id IN (#{answered_kirai_posts_id})", user_id: @user.id).page(params[:page] ||= 1).per(10).order('created_at DESC')
        page_length = Post.where("id IN (#{answered_kirai_posts_id})", user_id: @user.id).page(params[:page] ||= 1).per(10).total_pages
        json_data = {
      ...
      def not_interested_posts
        ...
        posts = Post.where("id IN (#{posts_id})", user_id: @user.id).page(params[:page] ||= 1).per(10).order('created_at DESC')
        page_length = Post.where("id IN (#{posts_id})", user_id: @user.id).page(params[:page] ||= 1).per(10).total_pages
        json_data = {
          'posts': posts,
          'page_length': page_length,
        }
      ...
      def user_posts
        @user = User.find_by(nickname: params[:nickname])
        user_posts = Post.where(user_id: @user.id).page(params[:page] ||= 1).per(10).order('created_at DESC')
        page_length = Post.where(user_id: @user.id).page(params[:page] ||= 1).per(10).total_pages
        json_data = {
          'posts': user_posts,
          'page_length': page_length,
        }
